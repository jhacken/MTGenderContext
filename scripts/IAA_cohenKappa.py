...
# Define function to calculate Cohens Kappa between annotators
# The function prints each IAA value between each pair and a total average
def calculate_cohensKappa_agreement(data):
    ...
    # Skip these sentences as they were not annotated
    skip_sentences = {"010": 'I cant even deal with this, one follower wrote alongside two fire emojis, while another wrote: "Love the hair x."',
                    "016": 'From bookkeeper to ski instructor.'}
    ...
    for annotator1, annotator2 in combinations(filenames, 2):
        ...
        if annotator1 in skip_sentences.keys() and annotator2 in skip_sentences.keys():
            pairwise_data = [item for item in annotation_data if item[0] in (annotator1, annotator2) and item[1] not in (skip_sentences[annotator1], skip_sentences[annotator2])]

        elif annotator1 in skip_sentences.keys():
            pairwise_data = [item for item in annotation_data if item[0] in (annotator1, annotator2) and item[1] != skip_sentences[annotator1]]

        elif annotator2 in skip_sentences.keys():
            pairwise_data = [item for item in annotation_data if item[0] in (annotator1, annotator2) and item[1] != skip_sentences[annotator2]]
    ...
    print(f"\nOverall Cohen's Kappa for all annotators: {overall_kappas}")
...
calculate_cohensKappa_agreement(data)